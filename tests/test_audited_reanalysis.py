...
class AuditedPopulationTests(unittest.TestCase):
    ...
    def test_preaccess_artifacts_and_hash_gate(self):
        ...
        if ar.RESULTS.exists():
            retrieval = json.loads((ar.RESULTS/"retrieval_manifest.json").read_text())
            self.assertEqual(retrieval["approved_preaccess_sha256"], digest); return
...
class AuditedEquationTests(unittest.TestCase):
    ...
    def test_sealed_result_manifest_and_schemas(self):
        if not ar.RESULTS.exists(): self.skipTest("preaccess stage")
        manifest = json.loads((ar.RESULTS/"retrieval_manifest.json").read_text())
        schemas = json.loads(ar.SCHEMAS.read_text())
        self.assertEqual(set(manifest["output_sha256"]), {path.name for path in ar.RESULTS.glob("*.csv")})
        for name, digest in manifest["output_sha256"].items():
            table = pd.read_csv(ar.RESULTS/name); spec = schemas[name]
            self.assertEqual(ar.sha256(ar.RESULTS/name), digest)
            self.assertEqual((len(table), list(table)), (spec["rows"], [item[0] for item in spec["columns"]]))
