...
the controlling numerical uncertainty. Aggregation across a fixed slope
threshold is. Neither area measure is used below to infer a global incidence
density or hazard distribution.

The registered geometric choices also remain influential. Across the three
windows, the 30~m sensitivity calculations span 0.27--114.0~km$^2$ for
glacier-contact area and 13.6--403.4~km$^2$ for PZI-intersection area as slope,
contact distance, and PZI threshold vary. These ranges describe the screens;
they are not uncertainty intervals or alternative opportunities to pass the
convergence test.
