...
included, trigger-time-eligible occurrences that have coordinates. For the
...
included, trigger-time-eligible occurrence with finite coordinates, giving 29
occurrences. Hourly 2-m temperature comes from the public temporal-layout
...
The coordinate requirement retains 29 of the 53 trigger-time-eligible
occurrences, and coordinate completeness is strongly source-dependent:
coordinates are present for 2 of 20 records from the independent rock-search
suite, 11 of 12 from the landslide-tsunami frame, and all 7 from the High
Mountain Asia frame. The summaries below therefore characterize the
coordinate-complete subset, not all 53 eligible occurrences.
...
before 00:00 UTC on the published catalog date. An audit found that these date
labels are not uniformly verified UTC: in particular, the Aru-2 local date
...
and 0.533 (0.300--0.759); 21 and 16 values exceed 0.5. The component-
representative medians are 0.621 and 0.552. Buffering changes event-year mean
...
screening, and registered ERA5 temperature pilot. The unresolved records
require primary-source follow-up, but they are not silently removed. The pilot
inspected only its registered temperature endpoints. The next association
analysis must acquire matched non-failing slopes and freeze glacier-contact
...
counts. Coordinate availability also selects a source-dependent subset of the
53 eligible trigger-time records whose representativeness is unknown.
